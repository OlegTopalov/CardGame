...
public class AITurnMaker: TurnMaker {
	...
	{
		foreach (Table table in p.tables) {
			foreach (CreatureLogic cl in table.CreaturesOnTable) {
				if (cl.AttacksLeftThisTurn > 0) {
					int tableIndex = Random.Range (0, 3);
					// attack a random target with a creature
					if (p.otherPlayer.tables[tableIndex].CreaturesOnTable.Count > 0) {
						int index = Random.Range (0, p.otherPlayer.tables[tableIndex].CreaturesOnTable.Count);
						CreatureLogic targetCreature = p.otherPlayer.tables[tableIndex].CreaturesOnTable [index];
