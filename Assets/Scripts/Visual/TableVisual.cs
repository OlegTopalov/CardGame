...
public class TableVisual : MonoBehaviour 
{
    ...
    {
        get {
			TableVisual[] allTables = GameObject.FindObjectsOfType<TableVisual> ();
			return (allTables [0].CursorOverThisTable || allTables [1].CursorOverThisTable || allTables [2].CursorOverThisTable 
				 || allTables [3].CursorOverThisTable || allTables [4].CursorOverThisTable || allTables [5].CursorOverThisTable);
		}
    }

	public static int CursorOverTable {
		get {
			TableVisual[] allTables = GameObject.FindObjectsOfType<TableVisual> ();
			for (int i = 0; i < 6; i++) {
				if (allTables [i].CursorOverThisTable) {
					return i;
				}
			}
			return -1;
